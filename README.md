...
- 개발동기
  ...
  2. git에서의 Markdown의 경우 사용하고자 하는 문법을 구글에 검색해가며 사용해야했고, 
  ...
  4. Markdown의 편리함을 많은 개발자들에게 제공하고자 이 오픈소스를 개발하게 되었습니다.
...
### <H4>이 오픈소스를 사용하고자 하는 SW개발자에 도움이 되는 것
    1. Markdown 작성에 대한 실시간 동기화를 제공하기 때문에 편리하게 Markdown을 사용 할 수 있다.
        몇몇 Markdown 편집기들은 preview를 지원하지 않기 때문에 자신이 작성하고 있는 Mardown code가 어떻게 보여지고 있는지 알 수 없어서 작업의 속도가 느리며,
    ...
    2. Markdown 문법의 어려움을 최소화해서 입문하기 부담이 없고, Word 기능을 추가해 익숙한 환경속에서 문서의 스타일 편집할 수 있다.
        입문 개발자들에게 가장 어려운 것은 언어의 어려움일 것이다. 새로운 언어를 터득하고 익숙해 지는데에 걸리는 시간은 절대 짧지 않다. 그렇기 때문에 중간에 포기를 
        하는 입문 개발자들이 생겨나기도 하고  편리한 기능을 두고도 이용하지 못하는 개발자들도 다수 생긴다. 이러한 문제점을 없애기 위해 우리는 Markdown의 문법을 더욱 
        쉽게 간소화하였다.
        Markdown의 문법을 이제 막 배우는 개발자라도 이 편집기를 사용하면 Markdown으로 편리하게 근사한 결과물을 만들어 낼 것이다. 더불어  Word에서 사용하는 스타일 편집
        기능 또한 지원하므로 더욱 완벽한 결과물을 만들 수 있다. 
        스타일 편집에 사용되는 편집 툴은 새로운 편집 툴이 아닌 현재 많은 사람들이 사용하는 익숙한 Word의 편집 기능을 가져옴으로써 익숙한 환경 속에서 작업할 수 있다.
    
    3. Mac OS에서 많은 기능을 가진  Markdown 편집기는 유료가 대부분인데, 이 편집기를 사용하면 무료로 많은 기능을 사용할 수 있다.
        Markdown 편집기들은 각각 다양한 기능을 제공한다. 무료인 편집기도 다수 있지만, 유료인 편집기도 존재한다. 특히, Mac OS에서 개발자들에게 칭찬받고 유명한 편집기들은
        대부분 유료이다. 따라서 이 편집기들의 기능을 사용하기 위해서는 비용을 지불해야 한다는 단점이 존재했다. 
        우리는 이 편집기에 Mac OS의 유료인 Markdown 편집기들의 기능을 추가함으로써 개발자들이 비용에 대한 부담없이 기능을 사용할 수 있도록 하였다.
    
    4. Preview를 오프라인 상황에서도 제공하므로 별도의 네트워크 연결이 없이도 코드 작성이 가능하고 더불어 문서화 가능하다.
        IT 강국 한국에서도 항상 온라인 상황에 있을 수는 없다. 불가피하게 오프라인 상태에서 문서를 작성해야 할 때가 반드시 존재한다. 이 편집기를 사용한다면 오프라인  상황
        에서도 Preview를 활용하면서 Markdown 작업을 할 수 있다.
