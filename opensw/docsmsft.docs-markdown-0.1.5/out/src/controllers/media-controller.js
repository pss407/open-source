...
function insertVideo() {
    telemetry_1.reporter.sendTelemetryEvent("command", { command: telemetryCommandMedia + ".video" });
...
function Insert(isArt) {
    ...
    if (!editor) {
        ...
        if (isArt) {
            ...
            actionType = "Link";
            telemetry_1.reporter.sendTelemetryEvent("command", { command: telemetryCommandLink + ".internal" });
        }
