...
function insertNumberedList() {
    telemetry_1.reporter.sendTelemetryEvent("command", { command: telemetryCommand + ".numbered" });
...
function insertBulletedList() {
    telemetry_1.reporter.sendTelemetryEvent("command", { command: telemetryCommand + ".bulleted" });
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        common_1.noActiveEditorMessage();
        return;
